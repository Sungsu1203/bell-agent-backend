...
from typing import TypedDict, List, Any, Mapping, cast, MutableMapping, Iterable, Optional
    ...
    if out_path:
        ...
        if should_halt:
            ...
            if not has_pending(tasks, writer, prefix="write"):
                ...
                _ = schedule_writer_if_needed(
                    cast(MutableMapping[str, Any], state),
                    tasks=tasks,
                    messages=msgs,
                    outline_text=outline_text,
                    allow_during_research=True,   # 조기 종료이므로 즉시 집필 허용
                    debug=True,
                )
