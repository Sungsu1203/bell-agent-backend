...
def supervisor(state):
    ...
    if target_from_line:
        ...
        if refs_empty or has_pending_rag:
            ...
            if not _safe_has_pending(tasks, WRITER_AGENT, prefix="write:"):
                tasks.append(Task(agent=WRITER_AGENT, done=False, description=f"write: {target_from_line}", done_at=""))
                logger.info("[Supervisor fast-path] writer pre-scheduled (fallback) → %s | title=%s", WRITER_AGENT, target_from_line)
        ...
        if did:
            messages.append(AIMessage(content=f"[Supervisor fast-path] → {WRITER_AGENT} (mode={DOC_MODE}, write: {target_from_line})"))
    ...
    if task.agent in ("chapter_writer", "section_writer"):
        ...
        if did:
            messages.append(AIMessage(content=f"[Supervisor reconcile] writer 예약 완료 (mode={DOC_MODE}, requested={requested or '(auto)'})"))
