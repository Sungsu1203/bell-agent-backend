...
def get_web_search_prompt() -> PromptTemplate: 
    return PromptTemplate.from_template(
        ...
        [검색어 생성 규칙(중요)]
        1) **각 쿼리마다 다음 바이어스(신뢰 도메인 선호)를 괄호로 그대로 덧붙인다.**
           (site:go.kr OR site:re.kr OR site:iea.org OR site:oecd.org OR site:kdi.re.kr)
        2) **각 쿼리마다 다음 금칙을 그대로 덧붙인다.** 
           -site:facebook.com -site:instagram.com -site:myfair.co -event -exhibition -tickets
           (한국어 잡음도 함께 배제: -행사 -티켓)
        3) 사용자가 행사/티켓/SNS 자체를 명시적으로 요청한 경우에만 해당 쿼리 1개에 한해 금칙을 생략해도 된다.
        4) 쿼리는 **구체적 개념 + 시점(예: 2025)** 를 포함하도록 작성한다.
        5) 같은 의미의 중복 쿼리는 생성하지 말고, 영역을 분리(정책/시장규모/공급망/기술동향 등)하라.
...
def get_chapter_writer_prompt() -> PromptTemplate:
    return PromptTemplate.from_template(
        ...
        [타깃 일치 규칙(매우 중요)]
        - 본문은 반드시 **[작성 대상 챕터]의 헤딩**을 기준으로 작성한다.
        - [작성 대상 챕터]가 '세부 목차 제안'·'목차'·'Outline'처럼 메타 헤딩이면:
