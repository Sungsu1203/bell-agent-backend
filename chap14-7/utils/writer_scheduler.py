...
from core.paths import now_str as _now_str # <- 이 라인 추가
...
def schedule_writer_if_needed(
    ...
    *,
    ...
    # 5) 예약 (중복 방지)
    has_writer_pending = has_pending(tasks_iter_for_check, writer_agent, prefix="write:")
    is_explicit_request = bool(req) # req는 요청된 제목 (req_param or flag_req)
    if has_writer_pending and is_explicit_request:
    # 🚨 수정 로직: 명시적 요청이 있고, 기존 writer 태스크가 펜딩 중이면,
    # 기존 태스크를 취소(done=True)하고 새 태스크를 예약합니다.

        now = _now_str()
        for t in task_list:
            if (not t.done) and t.agent == writer_agent and (t.description or "").startswith("write:"):
        ...
        # 이제 기존 태스크가 취소되었으므로, 아래에서 새 태스크를 예약합니다.
        has_writer_pending = False # 강제로 False로 설정하여 아래 if 문 통과 유도
