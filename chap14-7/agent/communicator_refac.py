...
def communicator(state: State):
    ...
    # ----------------------------------------------------------------------------------
    # 이 로직을 show_outline보다 앞에 배치해야 QA를 우선 처리할 수 있습니다.
    if state.get("qa_direct_reply"):
        ...
        if last_ai_msg and getattr(last_ai_msg, "content", ""):
            ...
            # QA 답변을 콘솔에 직접 출력 (Human-readable log가 켜져 있을 때도 보장)
            try:
                sys.stdout.flush()
            except Exception:
    ...
    if show_outline_req:
        ...
        return {"messages": messages, "task_history": tasks}
    # --- [show_outline_req 블록 끝] ---

    # 💡 [FIX] fallback_outline 정의 위치 조정: LLM 호출 전에 항상 정의되도록 보장
    fallback_outline = get_topic_outline_text(state)

    # 일반 커뮤니케이션 (Direct QA에서 처리되지 않은 경우만 실행)
