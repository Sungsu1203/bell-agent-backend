...
def communicator(state: State):
    def _inject_requested_title_if_needed(state: State):
        """
        writer 스케줄러가 REQUIRE_EXPLICIT_WRITE_TITLE=True 일 때
        requested_write_title 이 비어 있으면 자동으로 타이틀을 주입해 writer 블로킹을 해제한다.
        """
        try:
            ...
            # 주입 타이틀 생성
            topic_title = (state.get("topic_title") or "벤포벨 시장 리포트").strip()
            iter_no = int((state.get("iteration_count") or 0) or (state.get("flags") or {}).get("iteration_count") or 1)
    ...
    def _safe_save_report(state: State, content_hint: str | None = None):
        ...
        try:
            ...
            out_root = os.getenv("REPORT_OUT_DIR")
            out_root = Path(out_root) if out_root else (Path(current_path) / "outputs")
