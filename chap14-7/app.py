...
# 2. **해결책:** .env 파일에 설정된 값을 강제로 다시 로드하여 덮어씁니다.
#    dotenv의 get_key를 사용해 .env 파일의 원본 값을 가져와 강제 주입합니다.
try:
    from dotenv import get_key
    # .env 파일에서 "LOCAL_RAG_GLOBS" 키의 값을 가져옵니다.
    # 이 값은 외부 환경변수에 의해 덮어씌워지지 않은, .env 파일의 원본 값입니다.
    local_rag_globs_from_env = get_key(dotenv_path, "LOCAL_RAG_GLOBS")

    # .env 파일에 값이 있고, 현재 환경 변수 값이 다르다면 강제 덮어쓰기
    ...
    # get_key를 사용할 수 없거나 오류가 발생한 경우, 현재의 디버그 라인만 유지
    print(f"[CRITICAL DEBUG] Failed to force override: {e}", flush=True)


# 3. 최종 확정된 값으로 다시 확인 (이전 요청 디버그)
...
# 💡 [필수 FIX] 로컬 RAG GLOBs 환경 변수가 다른 곳에서 덮어씌워지는 문제 해결 💡
# LOCAL_RAG_GLOBS가 설정되어 있지 않을 때만 기본값 설정
# if not os.getenv("LOCAL_RAG_GLOBS", "").strip():
#     os.environ["LOCAL_RAG_GLOBS"] = "refs/*.pdf,refs/**/*.pdf,refs/*.txt,refs/**/*.pptx,refs/**/*.xlsx" # .env의 전체 기본값

# 💡 [디버그 로그 추가] LOCAL_RAG_GLOBS 로드 직후 값 확인 💡
import logging
logging.getLogger("__main__").info(
    "[DEBUG.ENV] LOCAL_RAG_GLOBS (After load): %r", 
    os.getenv("LOCAL_RAG_GLOBS")
)
# ───────────────────────────────────────────────────────────────────────────
...
def setup_logging(stream: Optional[TextIO] = None) -> None:
    ...
    # 3rd-party verbose 로그 추가 억제
    logging.getLogger("urllib3").setLevel(logging.WARNING)
    logging.getLogger("google").setLevel(logging.WARNING)   # google_auth, google_genai 등 포함
    # 💡 [추가] unstructured 라이브러리의 상세 추적(DETAIL/DEBUG) 로그 억제
    # unstructured는 trace 메시지를 DETAIL 레벨로 출력하는 경우가 많습니다.
    logging.getLogger("unstructured").setLevel(logging.WARNING) # 👈 이 줄을 추가
    # 선택: 필요할 때만 켜세요
    ...
    # app.py — setup_logging() 내부에 있는 HumanOnlyFilter를 아래로 교체
    import re as _re

    class HumanOnlyFilter(logging.Filter):
        """
        콘솔 최소화:
        - 기본: 핵심 배너/유저입력/리포트 알림만 통과
        - HUMAN_LOGS_STRICT=1 이면 오직 User/REPORT/Goodbye! 등만 통과
        """
        ...
        def filter(self, record: logging.LogRecord) -> bool:
            ...
            # 기본모드: agent/tools/utils 네임스페이스는 차단
            ...
            # 그 외는 INFO 이상만 __main__/report_builder 에서만 허용
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # app.py — state 병합 직후, 마지막 AI 발화만 콘솔로 출력
            try:
