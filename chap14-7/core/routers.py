...
def _metrics_snapshot_and_alert(state):
    ...
    try:
        ...
        if _metrics_snapshot:
            ...
            base_dir = os.path.join("resources", topic_slug)
            os.makedirs(base_dir, exist_ok=True)
            snapshot_path = os.path.join(base_dir, f"metrics_{int(time.time())}.json")
...
def after_web_search_agent(state: State):
    """웹 검색 완료 후 라우팅: 연구 루프가 활성 상태면 Synthesizer로 연결합니다."""
    tasks = state.get("task_history", []) or []

    # 1) 연구 루프가 활성화되어 있고, 쿼리가 실행되었다면 (research_planner -> web_search)
    if bool(state.get("research_loop_active")) and not has_pending(tasks, "research_synthesizer"):
        logger.info("[router.after_web] research loop active → research_synthesizer")
        # 💡 [핵심] web_search_agent의 결과는 반드시 Synthesizer로 넘겨서 분석하도록 강제합니다.
        return "research_synthesizer"
    
    # 2) 그 외 (일반 검색 등)는 tail router로 넘김
    nxt = tail_task_router(state)
    logger.info("[router.after_web] fallback to tail router → %s", nxt)
    return nxt
...
def after_vector_router(state: State):
    ...
    # 0) 연구 모드 강제 합성 라우팅
    #    - 연구 중에는 qa_direct_reply에 의한 커뮤니케이터 단락을 허용하지 않고,
    #      항상 합성기로 이어가도록 강제한다.
    if bool(state.get("research_loop_active")):
        # QA 단락 플래그를 내려 부작용 방지(후속 라우터에서 새지 않도록)
...
def after_synthesizer_router(state: State):
    ...
    if has_pending(tasks, preferred_writer, prefix="write:") or has_pending(tasks, alt_writer, prefix="write:"):
        ...
        # [ADD] 메트릭: 합성 이후 writer로 즉시 전이되는 경우도 라운드 종료로 간주하여 스냅샷/알람
    ...
    # [ADD] 연구 중단(라운드 종료) 시점 → 스냅샷 & 임계치 알람
    _metrics_snapshot_and_alert(state)
