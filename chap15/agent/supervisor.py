...
def _has_writer_pending_any(tasks, prefix: str | None = "write:") -> bool:
    try:
        for a in WRITER_ALIASES:
            if _safe_has_pending(tasks, a, prefix=prefix):
                return True
        return False
    except Exception:
        for t in (tasks or []):
...
def _tcount(tasks) -> dict:
    ...
    for t in (tasks or []):
        ...
        elif a in ("writer","chapter_writer","section_writer"): d["writer"] += 1
...
def _ensure_agent_env(state: MutableMapping[str, Any]) -> None:
    ...
    state["agent_role"] = raw_role.strip() if isinstance(raw_role, str) and raw_role.strip() else _env_str("BLOCKAGI_AGENT_ROLE", "")
    state["iteration_count"] = coerce_int(state.get("iteration_count", os.getenv("ITERATION_COUNT")), default=0)
...
def supervisor(state):
    ...
    if state.get("qa_direct_reply"):
        _flags = state.get("flags") or {}
        has_writer_pending = _has_writer_pending_any(tasks, prefix="write:")
    ...
    if _is_research_mode_local(state):
        ...
        if not has_research_pending:
            ...
            for t in tasks:
                if (not t.done) and t.agent in ("writer","chapter_writer","section_writer"):
                    t.done, t.done_at = True, now
                    t.description = (t.description or "") + " [auto-closed: start research loop]"
    ...
    if target_from_line:
        ...
        if refs_empty or has_pending_rag:
            ...
            if not _has_writer_pending_any(tasks, prefix="write:"):
                _mode = _env_doc_mode()
                scheduled_writer = "section_writer" if _mode == "report" else "chapter_writer"
                tasks.append(Task(agent=scheduled_writer, done=False, description=f"write: {target_from_line}", done_at=""))
                logger.info("[Supervisor fast-path] writer pre-scheduled (fallback) → %s | title=%s", scheduled_writer, target_from_line)
        ...
        if did:
            messages.append(AIMessage(content=f"[Supervisor fast-path] → writer (mode={_env_doc_mode()}, write: {target_from_line})"))
            _dash_emit(state, where="supervisor", picked="writer", reason="write_with_refs")
        else:
    ...
    if task.agent in ("chapter_writer", "section_writer"):
        ...
        if did:
            messages.append(AIMessage(content=f"[Supervisor reconcile] writer 예약 완료 (mode={_env_doc_mode()}, requested={requested or '(auto)'})"))
            if requested:
...
def supervisor_router(state) -> str:
    ...
    def _has_writer_any(prefix: str | None = "write:") -> bool:
        for a in WRITER_ALIASES:
            if _has(a, prefix=prefix):
                return True
        return False

    flags = state.get("flags") or {}
    has_writer_p = _has_writer_any(prefix="write:")
    ...
    if write_title:
        ...
        ret = "writer"; _dash_emit(state, where="router", picked=ret, reason="write_refs_present"); return ret

    if _is_research_mode_local(state):
        if _has_writer_any(prefix="write:"):
            pass
    ...
    elif _has_writer_any(prefix="write:"):
        ret = "writer"
    ...
    elif _has_writer_any(prefix="write:"):  # 일관화
        ret = "writer"
