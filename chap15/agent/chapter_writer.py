...
def _env_doc_mode(default: DocMode = _cast(DocMode, "book")) -> DocMode:
...
def _resolve_title(state: State, outline_text: str | None) -> tuple[str | None, bool]:
    """
    챕터 제목 결정 순서:
    1) flags.requested_write_title (락 우선)
    2) get_last_write_target(messages, tasks)
    3) next_unwritten_title(outline_text, mode='book')
    반환: (title, from_lock: bool)
    """
    try:
...
def chapter_writer(state: State):
    ...
    # ── book 모드에서만 동작
    if _env_doc_mode() != _cast(DocMode, "book"):
        logger.info("[CHAPTER WRITER] Skipped: DOC_MODE=%s (expected 'book')", _env_doc_mode())
    ...
    # ✅ 섹션 라이터와 동일한 타이틀 결정 규칙(+작성 요청 락 고려)
    ...
    # 진행 카운트: flags에만 저장(TypedDict 경고 회피) + 중복 카운트 방지
    try:
