...
def _has_writer_pending(tasks, prefix: str | None = None) -> bool:
    return any(
        (not getattr(t, "done", False))
        and getattr(t, "agent", "") in WRITER_ALIASES
        and (prefix is None or (getattr(t, "description", "") or "").startswith(prefix))
        for t in (tasks or [])
    )

def _latest_pending_writer_label(tasks) -> str:
    """
    로깅용 헬퍼. 실제 반환은 항상 'writer'지만,
    최신 펜딩 writer 라벨(섹션/챕터) 추정값을 로깅 메시지에 사용.
    """
    for t in reversed(tasks or []):
        if (not getattr(t, "done", False)) and getattr(t, "agent", "") in WRITER_ALIASES:
            return getattr(t, "agent", "writer")
    return "writer"
...
def tail_task_router(state: State):
    ...
    if outline_not_shown:
        ...
        if flags.get("pending_write_title") and _has_writer_pending(tasks, prefix="write:"):
            est = _latest_pending_writer_label(tasks)
            logger.info("[router.tail] outline exists but not shown — writer lock → %s → writer", est)
            return "writer"
        logger.info("[router.tail] outline exists but not shown → communicator")
        return "communicator"

    # writer-락 우선 (refs 가드 포함)
    refs = state.get("references") or {}
    refs_empty = not (refs.get("docs") or [])

    if _has_writer_pending(tasks, prefix="write:"):
        if refs_empty:
            logger.debug("[router.tail] write: pending but refs empty → web_search_agent")
            return "web_search_agent"
        est = _latest_pending_writer_label(tasks)
        logger.debug("[router.tail] writer pending(write:) → %s → writer", est)
        return "writer"
    ...
    # 기본: writer
    logger.info("[router.tail] no pending tasks → writer")
    return "writer"
...
def after_vector_router(state: State):
    ...
    # refs 가드: refs 비면 write: 펜딩이어도 웹검색으로
    if _has_writer_pending(tasks, prefix="write:"):
        refs = state.get("references") or {}
        refs_empty = not (refs.get("docs") or [])
        if refs_empty:
            ...
            return "web_search_agent"
        est = _latest_pending_writer_label(tasks)
        logger.info("[router.after_vector] writer pending(write:) → %s → writer", est)
        return "writer"
...
def after_planner_router(state: State):
    ...
    # 1) writer 펜딩 최우선
    if _has_writer_pending(tasks):
        est = _latest_pending_writer_label(tasks)
        logger.info("[router.after_planner] writer pending → %s → writer", est)
        return "writer"
...
def after_synthesizer_router(state: State):
    ...
    if _has_writer_pending(tasks, prefix="write:"):
        est = _latest_pending_writer_label(tasks)
        logger.info("[router.after_synth] writer pending(write:) → %s → writer", est)
        return "writer"
