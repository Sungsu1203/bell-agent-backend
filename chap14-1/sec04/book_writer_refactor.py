...
def web_search_agent(state: State):
    ...
    queries: list[str] = []           # ← 여기서만 초기화 (아래에서 절대 다시 비우지 말 것)
    json_paths: list[str] = []
    ...
    if forced_queries:
        ...
        for q in forced_queries:
            try:
                ...
                print(f"[WARN] forced web_search failed: {q} -> {e}")
    ...
    for args in tc2:
        ...
        try:
            _, json_path = web_search.invoke({"query": q})  # (results, json_path)
            print("json_path:", json_path)
            json_paths.append(json_path)
            add_web_pages_json_to_chroma(json_path)
            queries.append(q)
    ...
    messages.append(AIMessage(
        f"[WEB SEARCH AGENT] 검색 완료({len(queries)}건). JSON 저장 및 Chroma 적재 완료."
