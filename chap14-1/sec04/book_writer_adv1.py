...
from tools_up import retrieve, web_search, add_web_pages_json_to_chroma, web_page_json_to_documents, save_chapter, save_section, next_unwritten_title
        ...
        for m in reversed(messages):
            try:
                ...
                if isinstance(m, HumanMessage):
                    mm = re.search(r"^\s*(write|작성|집필)\s*[:：]\s*(.+)$",
                                   (m.content or "").strip(), flags=re.IGNORECASE)
                    if mm:
...
def save_md_draft(title: str, content: str, mode: str = None, base_dir: str = None, current_path: str = None):
    """
...
def merge_refs(existing: dict | None, new_queries: list[str] | None, new_docs: list | None) -> dict:
    ...
    """
    from langchain_core.documents import Document  # 안전: 여기서 임포트해도 됨
    import hashlib
...
def supervisor(state: State):  # supervisor 에이전트
    ...
    if m:
        ...
        if target:
            ...
            # ★ DOC_MODE에 따른 writer 확정 (핵심 패치)
            mode = _doc_mode()
            writer_agent = "section_writer" if mode == "report" else "chapter_writer"

            quick_task = Task(agent=writer_agent, done=False, description=f"write: {target}", done_at="")
            task_history.append(quick_task)
            note = AIMessage(f"[Supervisor fast-path] → {writer_agent} (mode={mode}, write: {target})")
            messages.append(note)
            print(note.content)
            return {"messages": messages, "task_history": task_history}
    ...
    if task.agent in ("chapter_writer", "section_writer"):
        mode = _doc_mode()
        expected = "section_writer" if mode == "report" else "chapter_writer"
        if task.agent != expected:
            fix_note = AIMessage(f"[Supervisor reconcile] DOC_MODE={mode} → writer agent forced to {expected} (from {task.agent})")
...
def web_search_agent(state: State):  # ①
    ...
    # 미리보기 문서: tools_up.web_page_json_to_documents 사용
    new_docs_preview = []
    try:
    ...
    # ⑪-추가) vector_search_agent 자동 예약(중복 방지)
    ...
    return {
        "messages": messages,
        "task_history": tasks,
        "references": state["references"],  # ← merged_refs 대신 state 값을 그대로
    }
...
def vector_search_agent(state: State):
    ...
    references = state["references"]  # 동기화

    # 검색 결과 로그(merge 이후 기준)
    print('Queries:--------------------------')
    ...
    # ✅ 3) writer 선택
    writer_agent = "section_writer" if (os.getenv("DOC_MODE", "book").strip('"').lower() == "report") else "chapter_writer"

    # ✅ 4) 동일 writer 펜딩 있으면 중복 금지
...
def section_writer(state: State):
    ...
    # ----- DOC_MODE guard -----
    if _doc_mode_value() != "report":
        print(f"[SECTION WRITER] Skipped: DOC_MODE={_doc_mode_value()} (expected 'report').")
        return {"messages": state.get("messages", []), "task_history": state.get("task_history", [])}
    ...
    # 5) 파일 저장 (sections/ 폴더) — save_md_draft가 있으면 사용, 없으면 로컬 저장기 사용
    try:
        # 존재 여부 확인만 위해 이름 참조
        _ = save_md_draft  # type: ignore
        out_path = save_md_draft(
            target_title,
            gathered,
            mode=_doc_mode_value(),          # 'report' 고정 전달
            current_path=current_path
        )
    except NameError:
...
def communicator(state: State):
    ...
    # === [교체] 저장 경로 한 줄만 덧붙이기 (실제 Writer 로그 기반) ===
    try:
        # 이미 본문에 경로가 있으면 생략
        has_path_already = (isinstance(gathered, AIMessage) and (
            ("chapters\\" in gathered.content) or ("sections\\" in gathered.content) or
            ("chapters/" in gathered.content) or ("sections/" in gathered.content)
        ))
        ...
        if not has_path_already:
            ...
            # 가장 최근 AI 메시지들에서 Writer 로그를 뒤에서부터 검색
            for m in reversed(messages):
                if not isinstance(m, AIMessage):
                    continue

                # 1) 표준 저장 로그: [Section Writer] ... → <...\.md>
                m1 = re.search(r"\[(?:Section|Chapter)\s+Writer\].*?→\s*(.+?\.md)\s*$", m.content)
                if m1:
                    last_save_path = m1.group(1).strip()
                    break

                # 2) 이동 로그: [Section Writer] moved ... -> <...\.md>
            ...
            if last_save_path:
                ...
                # gathered가 AIMessage가 아닐 수 있어 안전하게 래핑
...
def tail_task_router(state: State):
    ...
    # 1) 가장 최신의 '미완료'이면서 우리가 허용하는 agent를 선택
    for t in reversed(state["task_history"]):
        if (not t.done) and t.agent in allowed:
            return t.agent
    # 2) 없으면 DOC_MODE에 맞는 writer로 기본 이동
    mode = (os.getenv("DOC_MODE", "book") or "book").strip('"').lower()
    return "section_writer" if mode == "report" else "chapter_writer"

# 기존 고정 에지 제거:
# graph_builder.add_edge("vector_search_agent", "section_writer")
...
graph_builder.add_conditional_edges(
    ...
    tail_task_router,
    {
        "chapter_writer": "chapter_writer",
        "section_writer": "section_writer",
        # 안전망: 혹시 다른 agent가 꼽히면 communicator로
        "communicator": "communicator"
    }
)
