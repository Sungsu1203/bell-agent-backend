...
def supervisor(state: State):  # supervisor 에이전트
    ...
    if re.search(_rag_re, last_text, flags=re.IGNORECASE):
        ...
        # web_search만 예약 (vector는 web이 끝나면 예약)
        task_history.append(
            Task(
    ...
    if m:
        ...
        if target:
            ...
            note = AIMessage(
                f"[Supervisor fast-path] → {writer_agent} (mode={_doc_mode()}, write: {target})"
            )
            messages.append(note)
            print(note.content)
            return {"messages": messages, "task_history": task_history}
    ...
    if task.agent in ("chapter_writer", "section_writer"):
        ...
        if task.agent != expected:
            fix_note = AIMessage(
                f"[Supervisor reconcile] DOC_MODE={_doc_mode()} → writer agent forced to {expected} (from {task.agent})"
            )
...
def communicator(state: State):
    ...
    if show_outline_req:
        ...
        if not outline_text:
            default_by_mode = "outline_report.md" if _doc_mode() == "report" else "outline_book.md"
