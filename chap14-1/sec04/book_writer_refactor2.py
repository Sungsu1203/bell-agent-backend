...
"""
# put these two lines at the very top, right after the docstring
import os
os.environ.setdefault("USER_AGENT", "book-writer-refactor/1.0 (+contact: your-email@example.com)")
...
# 안전 스니펫 / 에러 상수
_ERR_TOO_LARGE = "max_tokens_per_request"

def _clean_snip(s: str, maxlen: int = 350) -> str:
    if not s:
        return ""
...
def _refs_preview_text(state: "State", max_q=5, max_docs=8, snippet_len=350) -> str:
    ...
    for d in docs:
        ...
        snip = _clean_snip(d.page_content or "", snippet_len)
...
def get_last_write_target(messages, tasks) -> Optional[str]:
    ...
    for m in reversed(messages):
        if isinstance(m, HumanMessage):
            mm = RE_WRITE_INLINE.search(m.content or "")
            if mm:
...
def _parse_force_queries_from_text(text: str) -> list[str]:
    """`force_queries: [ ... ]` 형태를 안전 파싱"""
    try:
        m = re.search(r"force_queries\s*:\s*(\[[\s\S]*\])", text, re.IGNORECASE)
        if not m:
            return []
...
def supervisor(state: State):
    ...
    fqs = _parse_force_queries_from_text(last_text)
    if fqs:
        task_history.append(Task(agent="web_search_agent", done=False, description=f"rag_update:auto force_queries={json.dumps(fqs, ensure_ascii=False)}", done_at=""))
...
def web_search_agent(state: State):
    ...
    def _run_web_search_with_guard(q: str,
                                queries: list[str],
                                json_paths: list[str],
                                new_docs_preview: list,
                                preview_limit: int = 5,
                                retries: int = 2) -> bool:
        """web_search를 안전하게 실행하고, 미리보기 문서까지 채워 넣는다."""
        for attempt in range(retries + 1):
            try:
                ...
                try:
                    ...
                    for d in docs:
                        ...
                        new_docs_preview.append(
                            Document(page_content=(d.page_content or "")[:500],
                                    metadata={"source": src})
                        )
                except Exception as e:
                ...
                time.sleep(1.2 * (attempt + 1))  # empty → 짧게 재시도
            except Exception as e:
                if attempt < retries:
    ...
    # ---------- 강제 쿼리 추출 ----------
    forced_from_desc = _parse_force_queries_from_text(mission)
    forced_from_msgs = extract_forced_queries_from_messages(messages, lookback=20)
    forced_queries: list[str] = []
    for q in (forced_from_desc + forced_from_msgs):
        if q and q not in forced_queries:
            forced_queries.append(q)

    # 재시도 방지용 집합(성공/실패 모두 기록)
    ...
    def _run_search(q: str):
        ...
        attempted.add(q)
        try:
            _ok = _run_web_search_with_guard(q, queries, json_paths, new_docs_preview)
        except Exception as e:
            msg = str(e)
            print(f"[WARN] web_search 실패: {q} -> {msg}")
            failed.add(q)
    ...
    if auto_mode and not queries and not forced_queries:
    ...
    try:
        for p in json_paths[:3]:
            ...
            for d in docs:
                ...
                src = meta.get("source") or meta.get("url") or "unknown"
                new_docs_preview.append(
                    Document(page_content=_clean_snip(d.page_content or "", 500), metadata={"source": src})
                )
    except Exception as e:
    ...
    if not has_pending(tasks, "vector_search_agent"):
        ...
        if queries:
            desc += f" queries={json.dumps(queries, ensure_ascii=False)}"
        if json_paths:
            desc += f" json_paths={json_paths}"
        if forced_queries:
            desc += f" force_queries={json.dumps(forced_queries, ensure_ascii=False)}"
        tasks.append(Task(agent="vector_search_agent", done=False, description=desc, done_at=""))

    # 요약 메시지
    fail_note = f" / 실패 {len(failed)}건 스킵" if failed else ""
    messages.append(AIMessage(
        f"[WEB SEARCH AGENT] 검색 완료({len(queries)}건){fail_note}. JSON 저장 및 Chroma 적재 완료."
...
def vector_search_agent(state: State):
    ...
    # 펜딩이 없더라도 계속 진행(방어)
    if pending is None:
        print("[WARN] vector_search_agent pending task가 없습니다. 방어적으로 진행합니다.")
        class Dummy:
            description = ""
        pending = Dummy()
    ...
    vector_search_system_prompt = PromptTemplate.from_template(
        """
        너는 다른 AI Agent 들이 수행한 작업을 바탕으로, 
        목차(outline) 작성/보고서 집필에 필요한 정보를 벡터 검색으로 찾아내는 Agent이다.
    ...
    mission = (getattr(pending, "description", "") or "")
    references = state.get("references", {"queries": [], "docs": []})
    messages = state.get("messages", [])
    outline_text = get_outline(current_path)

    inputs = {"mission": mission, "references": references, "messages": messages, "outline": outline_text}
    ...
    for doc in references["docs"]:
        print(_clean_snip(getattr(doc, "page_content", "") or "", 100))
        print("--------------------------")

    # 완료 표기(있을 때만)
    if hasattr(pending, "description"):
        pending.done = True
        pending.done_at = _now_str()

    # --- 집필 예약 로직 보정 ---
    # force_queries 기반 수집(=사용자 RAG 갱신 요청)일 때는 자동 집필 금지하고, 커뮤니케이터로 요약 보고만.
    mission_desc = mission
    # 메시지에도 강제쿼리 흔적이 있으면 RAG-only
    is_manual_force = ("force_queries=" in mission_desc) or bool(extract_forced_queries_from_messages(messages, lookback=20))
    ...
    writer_agent = WRITER_AGENT
    if is_manual_force and not requested_title:
        if not has_pending(tasks, "communicator"):
            tasks.append(Task(agent="communicator", done=False, description="RAG 업데이트 결과 요약 및 다음 작업 확인", done_at=""))
    else:
        if not has_pending(tasks, writer_agent, prefix="write"):
...
def chapter_writer(state: State):
    ...
    # 집필 대상 결정
    target_title = get_last_write_target(messages, tasks) or next_unwritten_title(outline_text)
    if not target_title:
    ...
    inputs = {"target_title": target_title, "outline": outline_text, "references": ref_text, "messages": messages}
...
def communicator(state: State):
    ...
    communicator_system_prompt = PromptTemplate.from_template(
        ...
        절대 확정되지 않은 사실을 단정하지 말고(예: '모든 섹션 초안 완료'), 
        실제 task_history나 last_saved_path 근거만 요약하라.
    ...
    inputs = {"messages": messages, "outline": fallback_outline}
...
def tail_task_router(state: State):
    ...
    for t in reversed(state["task_history"]):
        if (not t.done) and t.agent in allowed:
            return t.agent
    # SAFE DEFAULT: fall back to communicator (not writer) to avoid stray writer runs
    return "communicator"
