...
중복 등재하지 않고 소재만 가리킨다.
- **catch CL** (`CLAUDE.md §9`) — 요약·대표값·첫 일치를 전수 대신 쓰면 답이 달라진다.
  판정에 쓰는 값이 **전수인지 표본인지**를 먼저 밝힌다.
- **catch CM** (`CLAUDE.md §9`) — 판단 불가는 **제3의 칸**에 넣는다. 어느 쪽으로도 밀지 않는다.
  미확정 칸의 크기가 결론을 바꿀 만하면 닫으러 가되, **기준이 아니라 자료를 바꾼다**.
- **catch CS** (`CLAUDE.md`) — 선언·인용은 검증이 아니다. 사본은 원본과 byte 대조 후에만 쓴다.
- **catch CT** (`CLAUDE.md`) — 원장 실물을 보지 않고 원장 규율을 지시하지 않는다. (챗 발)
...
>
> 🔴 **대조기 자체가 침묵 실패한다 (catch CR, 2026-08-10 §research-1 S6)**
> §8 은 *산출 경로*의 침묵 실패를 다루나, **대조하는 코드도 같은 방식으로 실패한다.**
> 파이썬 `zip` 은 짧은 쪽에서 조용히 멈춘다. 18줄 vs 17줄을 대조했는데 **"불일치 0건"** 이
> 출력됐다(18번째가 비교에서 빠짐). 에러·경고 0. 실제 원인은 파싱 오류(코드펜스 밖 `---` 을
> 1줄 더 집음)였고 **검사 결과에는 전혀 나타나지 않았다.**
> 발견 경로는 §8 과 동일 — **줄 수를 따로 찍어둔 덕에 `18 != 17` 이 눈에 걸렸다.**
> **절차 3항 (2항에 추가)**
> 3. 대조 루프 **앞에 `assert len(a) == len(b) == N`** 을 둔다. 길이를 맞춰 자르지 않는다.
>    같은 계열 = `RAG_DISTANCE_THRESHOLD` 폴백 미설정 + assert(`probe_q1_arms.py:129`) ·
>    `TOPIC_SLUG` 이중 assert 를 무거운 import 앞에(`catch AB`). **조용한 실패를 시끄러운 실패로 바꾼다.**
