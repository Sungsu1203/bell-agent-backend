...
| catch 52 | (MID — §academic-3 Step C-1 fix 본체 완료 (보수적 7 entries + 변수 재명명), Step C-2 측정 대기) `ACADEMIC_DOMAINS` set 보강 — 글로벌 학술 플랫폼 mdpi/researchgate/semanticscholar/academic.naver 등 후보 검토 | §academic-2 부수 미션 PARTIAL root cause 분리 박제. §academic-2 측정 raw (academic-en all_uniq) 에서 `mdpi.com` (5 runs 모두), `researchgate.net` (3 runs), `pdfs.semanticscholar.org` (1 run), `academic.naver.com` (5 runs 모두) 등이 vertex 결과 분포에 있으나 ACADEMIC_DOMAINS_29 set 미포함 → ratio 0.0. set 보강 시 academic-en ratio 회복 가능. 코드 변경 거의 0 (env / driver set 정의 만), 효과 명확 — **최우선 sub-cycle 후보**. 본 진입 trigger 조건 = §academic-2 close 직후. **§academic-3 진행 (2026-05-20)**: Step A entry audit (commit `10541d2`) 완료 — A1 set 정의 위치 (`measure_ab.py:138-146` + 3 토픽 .env `ALLOWED_DOMAINS_EXTRA`) + A2 academic-en 5 runs 49 도메인 분포 분석 (Q1 엄격) + A3 보강 후보 7 entries 박제. **사용자 결정 ①~④ 확정**: ① 보수적 7 entries 전수 수용 (HIGH 4 mdpi/researchgate/academic.naver/acr-journal + MID 3 sciencedirect/journalofadvertising/aom) · ② researchgate 학술 SNS 인정 (peer-review working paper 관행 정합) · ③ catch 45 분리 (본 cycle 은 catch 52 만) · ④ set 재명명 Step B 이월. Step B design (commit `8d6d2e4`) + follow-up (commit `ddc59a4`) 완료 — B1 옵션 A 단일 set + 9 카테고리 헤더 / B2 옵션 B `ACADEMIC_DOMAINS_29 → ACADEMIC_DOMAINS` / B3 §academic-2 동일 5 metric + `[GATEKEEP] n` (108 → 115 예상) / Risk 박제 (academic-en ratio 예상 ≈ 0.31, PARTIAL 가능, 잔존 영역 = catch 51 + catch 53). **Step C-1 fix 본체 완료**: measure_ab.py:137-146 set literal 재구성 + line 397 참조 site 변수명 갱신 + 3 토픽 .env `ALLOWED_DOMAINS_EXTRA` 라인 36 entries 갱신. 함수 본체/hook/신규 def/test 변경 0 (audit summary 정합). Step C-2 측정 대기. |
