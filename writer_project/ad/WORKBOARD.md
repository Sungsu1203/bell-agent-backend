...
| **임베딩 기술 불일치** | 🔴 **미해결 갈래 — 유료 단계 전 판정 필요.** **규범 문서**(`ad/GUARDRAILS.md:20` 한국어=`text-multilingual-embedding-002`(768d) · `:42` `text-embedding-004` 금지, 둘 다 `README-dev §3`·`§13` 요약)와 **`ad/README-dev.md:210`·`:1409`** 가 `002` 를 말하는데 **실효값은 `text-embedding-3-large`** 다. 두 갈래 중 어느 쪽인지 미판정 — **① 규정이 stale**(002 시절 문서가 안 고쳐짐) / **② 설정이 규정을 이탈**(문서가 정본인데 `.env.openai` 가 어긋남).<br>⚠️ **고치지 않는다(챗 결정 2026-08-09).** 한쪽을 골라 문서를 손대면 확정 안 된 판정을 박제하게 된다. **색인 X·Y 2벌 대조 착수 전에 판정**할 것 — 두 색인이 다른 임베딩 모델을 쓰면 차이가 크롬 제거 때문인지 모델 때문인지 분리되지 않아 대조 전체가 무효다.<br>🔴 **2026-08-09 실측 정정 — "문서 계통이 *일관되게* 002 를 말한다"는 성립하지 않는다.** 같은 `README-dev.md` 안에 **3-large 를 기술한 행이 4건 더 있다**: `:1368`·`:1371`(3-large + Chroma `hnsw.space=l2` 실측 분포) · `:2001` · `:3234`. 즉 실제 형태는 "규정 vs 설정"이 아니라 **한 파일 안의 신·구 기술 병존**이다. ①/② 판정 시 이 4행을 함께 볼 것 | **최상** |
