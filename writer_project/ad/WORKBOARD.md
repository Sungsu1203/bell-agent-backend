...
- **드라이버** `scripts/§research-1/run_r3a_straight.py` · **`probe_q1_arms.py`·`probe_q2_zdouble.py` = `writer_project/` 루트** (전부 untracked; probe 2건은 `.gitignore:106 probe_*`)
  ⚠️ **2026-08-09 정정** — 구 문안은 세 파일을 `scripts/§research-1/` 로 묶어 적었으나 **probe 2건은 그 경로에 없다.** 이 오기가 `Q2 §3.1` 이월 건의 원인이었다(위 해소 행)
