...
| **재색인 실측** | 🔴 **원자료에 본문이 없다** — `resources_*.json`이 118 source를 100% 커버하나 `raw_content` **0자 전건**, `content`는 37~215자 스니펫(색인 본문 766,607자). → **재색인 = URL 재사용 + HTML 재수집**(무료). 🔴 배치 = **46**, `CHROMA_MAX_BATCH=16`에 걸린 배치 **0/46**(64로 올려도 46) — 실제 제약은 하드코딩 `MAX_TEXT_SUM=18000`(`ingest_vector.py:1236`), **env로 못 바꾼다.** `INDEX_TIMEOUT_SEC=60`은 중단이 아니라 **사후 경고 로그**(`_batched_add:646-651`) → 시간 추정 근거 아님. ✅ **링크로트 GET 기준 115/118(97.5%) · 413/416청크(99.3%) 생존** → 재수집 성립. 잔여 3건은 SSL 미해결(죽은 링크 아님 — 논문 트랙 certifi 전례 확인 필요). R5 §4 · R6 §3 | **상** |
...
| **C 게이트** | 🔴 **`published_date` 비용이 분기마다 다르다.** 사본 ①(`ingest_docs.py:386`) = DOM `:385~390`, 반출 경로 **있음**(`:398` 메타 dict) → 한 줄. 사본 ②(`ingest.py:677`) = DOM `:675~682`, 반출 경로 **없음**(계약 `(url,timeout) -> str`) → 시그니처 변경 + 호출부 연쇄. **어느 쪽이 본선인지 미확정.** 폴백(except) 경로는 양쪽 다 정규식이라 DOM 부재 → 불가. ⚠️ 날짜 추출 코드 전 레포 0건, `soup.find`/`find_all` 0건 — **C는 확장이 아니라 신규 기능.** R6 §2 | 상 |
