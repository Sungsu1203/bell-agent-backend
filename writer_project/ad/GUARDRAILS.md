...
- **데이터 품질 운영 노하우** — HWP/이벤트·광고/SEO 시장리포트 노이즈 패턴,
  ...
  - 🔴 **정규 수집 산출물을 하류가 읽지 않는다 (catch CK, 2026-08-06 발견 / 2026-08-10 조문 등재)**
    ⚠️ **이 번호는 여러 차수 동안 인용만 되고 조문이 없었다**(`catch CS` ②). 소재를 이 항으로 고정한다.
    `web_search.py` 사슬 — `:846-849` path 폐기 → `:934 ret = combined_items`(list) →
    `:946-948 json_path=""` → `:958` 항상 True → `:964-968` **fallback 저장** → `:1049 _filter_json_by_domain`.
    🔴 정규 writer 산출 경로가 폐기되어 **fallback writer 가 항상 발동**한다.
    **이 버그가 `raw_content` 원본을 살리고 있다** — "고치면" `_to_safe_json_record` 4,000자 절단이
    되살아나 수집이 무의미해진다. **무접촉이 정본이다.**

    **2026-08-10 §research-1 S6 실측 — 2계보 병존**
    ...
    🔴 **정규 경로는 죽지 않았다. 파일을 산출하되 하류가 읽지 않는다.**
    → **파일명 규칙으로 계보를 판별할 수 있다.** 유료 수집분의 `raw_content` 보유 근거가 된다.
    ⚠️ 이 표는 자체 정정을 거쳤다 — 1차 집계가 `./resources` 하위만 세어 정규 산출물 **0건** 을
    얻었고 *"정규 경로는 산출물이 아예 없다"* 로 갈 뻔했다. 레포 루트 재탐색에서 72건
    (`CLAUDE.md §9` "양성 대조 없는 0건").
  ...
  - 🔴 **켜짐이 결과로 드러나지 않는 게이트 (catch CU, 2026-08-10 §research-1 S6 실측)** —
    `GATE_KEEP_SOURCES` 가 꺼져 있으면 `web_search.py:533-534` 가 원본 경로를 그대로 반환해
    **`*_filtered.json` 이 생성되지 않는다. 그런데 실행은 에러 없이 성공으로 끝난다.**
    유료 수집에서는 비용만 나가고 산출물이 없다.
    실측 = `.env:208 GATE_KEEP_SOURCES=0` · `topics/experiential-marketing-media.env` 설정 0건
    (`=1` 보유 프리셋은 academic 계열 4개뿐). CFG 선언 3건(`core/config.py:278`·`:511`·`:771`)
    → `.env` 로 점화 가능. 별도 수단 `app.py:2232-2234 --gatekeep`.
    ⚠️ `_env_flag` 는 엄격 파서로 `"0"`→False 정상 — **`catch 71`(truthy 오파싱) 유형이 아니다.**
    ⚠️ **`.env` 는 untracked 라 설정 변경 이력이 없다.** 꺼진 시점 판정의 유일한 증거가 probe 로그
    타임스탬프였다 — `probe_A2/B1.log`(8/1 17:02·17:07) enabled(n=79) vs `probe_C~F.log`
    (17:21~17:48) disabled → **8/1 17:07~17:21 사이.** 어느 수단으로 껐는지는 **미확정**(`catch CM`).
    → 수집 착수 전 **점화 + 로그 실동작 확인**까지가 게이트다. 설정값 확인만으로는 부족하다
...
- **catch 네임스페이스 2계열 — 섞지 않는다.** (2026-08-10 명문화)
  ...
  - **결번 `CQ`** (2026-08-10) — S6 에서 "측정 계보" 후보로 초안됐으나 `CLAUDE.md §9 catch CI`
    (탐침이 파이프라인과 다르면…)와 동일 사안이라 **CI 보강으로 흡수**했다. 재사용하지 않는다.
    (기존 결번 = `CC`~`CF`)
