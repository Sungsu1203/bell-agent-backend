...
>
> **미확정 관측 (catch 번호 미부여 — 챗 판정 대기)** — 그때 켠 수단이 `.env` 값인지 `--gatekeep` 인지는 판정 불가하다(`.env` untracked, 이력 없음). 어느 쪽으로도 밀지 않는다. 조치는 어느 쪽이든 동일하다 — **수집 직전 점화하고 로그로 실동작을 확인한다.**
...
> 🔴 **미확정 관측 1건 (catch 번호 미부여 — 챗 판정 대기)** — 어느 쪽으로도 밀지 않는다. 그때 켠 수단이 `.env` 값이었는지 `app.py --gatekeep` 였는지는 **판정 불가**다. `.env` 는 untracked 라 이력이 없다. 다만 **조치는 어느 쪽이든 동일**하다: 수집 전에 실효값을 켜고 로그로 확인한다. 미확정의 크기가 결론을 바꾸지 않으므로 열어 둔다.
...
| **78** | 행번호 재확인 | ✅ 아래 표 |
| **79** | 편집 3건 반영 (2026-08-10 fix 차수) | ✅ ① `CM` 표기 제거 + 전문 스캔(**추가 0건**) ② §10 서두 독립 블록 ③ §7-e 단서 3 |
...
> **fix 차수 catch 라벨 스캔 결과 (#79-①)** — R11 전문에서 CC 판단으로 새로 부여한 라벨은 **`CM` 1건뿐**이었고 제거했다. 나머지 문자 라벨(`CO`·`AR`·`CG`×2·`BX`·`CH`·`CK`×2·`BB`·`AS`·`CL`)과 숫자 라벨(`71`)은 **전부 기존 등재분 인용**이라 유지했다. §13 의 `CQ`·`CR` 은 **"catch 후보" 수식과 함께** 초안 상태로만 표기했고 확정 번호로 쓰지 않았다(본문 참조 3곳 `:87`·`:206`·`:427` 모두 "후보" 명시). **추가 제거 0건.**
