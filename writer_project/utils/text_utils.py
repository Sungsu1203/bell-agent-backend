...
__all__ = [
    ...
    "plain_snip",
    "refresh_text_utils",
    "section_slug_candidates", "extract_leading_index",
]
...
def extract_leading_index(title: str) -> Tuple[Optional[int], str]:
    """
    "6. Recommendations" → (6, "Recommendations")
    "6) Risks & Mitigations" → (6, "Risks & Mitigations")
    접두 숫자가 없으면 (None, 원문) 반환.
    """
    m = _LEADING_INDEX.match(title or "")
    if not m:
...
def _basic_slug(s: str) -> str:
    """작성기와 병합기의 규칙을 맞추기 위한 최소 슬러그 정규화."""
    s = (s or "").strip().lower()
    # &는 공백으로 치환하여 하이픈 연속 최소화 (예: "a & b" → "a b")
    s = s.replace("&", " ")
    # 공백 → 하이픈
    s = re.sub(r"\s+", "-", s)
    # 영숫자/한글/하이픈만 유지(유니코드 허용 기준과 정렬)
...
# 병합 탐색용 후보 슬러그들 생성
def section_slug_candidates(title: str) -> List[str]:
    """
    병합 시 파일 탐색 후보:
      1) "<n>-<slug>"         (예: 7-risks-mitigations)
      2) "0<n>-<slug>"        (예: 07-risks-mitigations)
      3) "<slug>"             (예: risks-mitigations)
    """
    idx, rest = extract_leading_index(title or "")
    base = _basic_slug(rest or title)
    cands = [base]
    if idx is not None:
        cands = [f"{idx}-{base}", f"{idx:02d}-{base}"] + cands
    return cands
