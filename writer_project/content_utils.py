# content_utils.py — dynamic config access (v2025-10-27)

from __future__ import annotations
from typing import List, Optional, Tuple, Literal, TypeAlias, cast, Protocol
...
# ─────────────────────────────────────────────────────────────
# Dynamic config access (avoid static binding to CFG types)
# ─────────────────────────────────────────────────────────────
import core.config as config

# 이 모듈에서는 DocMode를 외부 정의에 정적으로 묶지 않도록, 로컬 리터럴로 고정
DocMode: TypeAlias = Literal["report", "book"]

class TitleExtractor(Protocol):
    def __call__(self, __title: str) -> List[str]: ...


# ── section_slug_candidates: 안전한 기본값 주입 ─────────────────────────────
def _default_section_slug_candidates(title: str) -> List[str]:
    return []
try:
    from utils.text_utils import section_slug_candidates as _ssc  # 실제 구현
except Exception:
    _ssc = _default_section_slug_candidates
section_slug_candidates: TitleExtractor = _ssc
...
def save_md_draft(
    ...
    *,
    ...
    # report 모드(섹션)는 find_section_path와 동일 규칙으로 파일명을 생성
    if m == "report":
        # get_content_dir("report", ...) → <root>/sections/<topic_slug>
        base_dir = get_content_dir(m, root_dir=root_dir, topic_slug=topic_slug)
        # 표준 후보(번호/패딩 일관) 시도 → 실패/공백이면 안전 폴백
        try:
            cands = section_slug_candidates(title)
        except Exception:
            cands = []
        if cands:
            slug = cands[0]
            ...
            # 안전 폴백: 기존 단일 규칙으로
            from utils.text_utils import section_slugify  # lazy import
            slug = section_slugify(title)
        p = Path(base_dir) / f"{slug}.md"
    else:
        # book 등 기존 규칙 유지(챕터/기타 초안)
...
def save_section(
    ...
    *,
    ...
    """
    섹션 저장도 탐색 규칙과 100% 일치하도록 후보 슬러그의 첫 번째를 사용.
    """
    # get_content_dir("report", ...) → <root>/sections/<topic_slug>
    base_dir = get_content_dir("report", root_dir=root_dir, topic_slug=topic_slug)
    # 후보 생성 시도 → 실패/공백이면 기존 경로 규칙으로 폴백
    try:
        cands = section_slug_candidates(title)
    except Exception:
        cands = []
