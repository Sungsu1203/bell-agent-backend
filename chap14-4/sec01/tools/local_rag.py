...
def _read_html(path: str) -> str:
    ...
    return re.sub(r"<[^>]+>", " ", raw)         # <로 시작해서 >로 끝나는 덩어리를 잡아라
...
def _to_webjson_items(path: str) -> List[dict]:
    ...
    if ext == ".pdf":
        ...
        for i, txt in enumerate(pages, start=1):
            ...
            items.append({
                "title": f"{title} (p.{i})",
                "url": url_for_click,      # 사람이 눌러볼 주소
                "source": source_key,      # 디듀프 키
                "content": txt
            })
        if not items:
...
def build_webjson_from_local(globs: List[str], out_dir: str) -> str:
    ...
    if items:
        print(f"[LOCAL RAG] sample sources: {[it.get('source') for it in items[:3]]}")
