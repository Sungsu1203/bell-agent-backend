...
from typing import List, Optional, Literal, cast, Final, Mapping, Any, Required
...
def web_search_agent(state: State):
    ...
    COUNT_PREVIEW_URLS = os.getenv("COUNT_PREVIEW_URLS", "0") == "1"
    MAX_INDEXED_PER_ROUND = int(os.getenv("MAX_INDEXED_PER_ROUND", "0"))    # 0이면 제한 미적용
    ...
    # persist_dir = _topic_dir(state.get("topic_slug")) if state.get("topic_slug") else None
    slug = state.get("topic_slug")  # Optional[str]
    persist_dir = _topic_dir(slug) if slug else None

    chunk_total = 0
    added_preview_urls = set()

    # --- (2) 소스 게이트 설정(환경변수 기반) ----------------------------------
    # GATE_KEEP_SOURCES=1 일 때만 화이트리스트 도메인 필터 적용
    ...
    # 쉼표로 분리된 도메인 리스트를 읽고 정규화
    _env_allowed = os.getenv("ALLOWED_DOMAINS", "")
    if _env_allowed.strip():
        ALLOWED_DOMAINS = {d.strip().lower() for d in _env_allowed.split(",") if d.strip()}
    else:
        ALLOWED_DOMAINS = set(_DEFAULT_ALLOWED)

    def _allowed(src: str) -> bool:
        if not GATE_KEEP_SOURCES:
            return True
        s = (src or "").lower()
        # 부분포함 검사(하위 경로/서브도메인 허용)
        return any(dom in s for dom in ALLOWED_DOMAINS)
    
    def _load_items(json_path: str) -> list[dict]:
        """최상위가 list 또는 dict({'results': [...]}) 모두 지원.
        그 외에도 items/data 키, 단일 객체, NDJSON까지 최대한 흡수."""
    ...
    def _filter_json_by_domain(json_path: str) -> str:
        """web_search 저장 JSON에서 허용 도메인만 남긴 새 JSON을 만들어 경로 반환.
        GATE_KEEP_SOURCES=0이면 원본을 그대로 사용."""
        if not GATE_KEEP_SOURCES:
            return json_path

        # 통합 로더 사용: 리스트/딕셔너리/NDJSON 등 최대한 흡수
        try:
            items = _load_items(json_path)
        except Exception:
            return json_path  # 파싱 실패 시 원본 사용
    ...
    def _run_web_search_with_guard(q: str, preview_limit: int = 5, retries: int = 2) -> bool:
        nonlocal chunk_total, added_preview_urls
    ...
    if not dedup_globs:
        ...
        #     pat_abs = pat if pat.startswith(os.sep) or (":" in pat) else os.path.join(current_path, pat)
        #     found = list(glob.iglob(pat_abs, recursive=True))
        for pattern in dedup_globs:  # pat -> pattern (더 명확)
    ...
    new_url_count = chunk_total if COUNT_PREVIEW_URLS else chunk_total
...
def vector_search_agent(state: State):
    ...
    if user_q and not _is_noise_query(user_q):      # ✅ 가드 추가
        try:
    ...
    for args in iter_tool_calls(search_plans, "retrieve"):
        ...
        if _is_noise_query(query) or (not _ok_query(query)) or (query in ran_queries):   # ✅ 가드 추가
            continue
    ...
    for q in preexisting_queries:
        q = _clean_seed(q)
        if _is_noise_query(q) or (not _ok_query(q)) or (q in ran_queries):               # ✅ 가드 추가
            continue
