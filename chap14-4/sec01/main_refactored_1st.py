...
from content_utils import (
    ...
    parse_outline_headings,  # (미사용 시 유지: 하위 호환)
    is_written,              # (미사용 시 유지: 하위 호환)
    get_content_dir,         # (미사용 시 유지: 하위 호환)
    path_for_title,          # (미사용 시 유지: 하위 호환)
)
...
def _score_doc(d: Document) -> float:
    ...
    if y:
        yr = int(y.group(1))
        score += max(0, (this_year + 1 - yr) * 0.1)
    return score
...
def supervisor(state: State):
    ...
    if m_new:
        maybe_title = _sanitize_title((m_new.group("title") or "").strip() or "untitled report")
        ...
        print(msg)
        tasks.append(Task(agent="content_strategist", done=False, description=f"create_outline:{state['outline_fname']}", done_at=""))
        return {**state, "messages": messages, "task_history": tasks}

    # 연구 라운드 모드 부트스트랩 (명시적 목차 요청 제외)
    if (
    ...
    # content_strategist pending 우선 처리
...
def web_search_agent(state: State):
    ...
    MAX_INDEXED_PER_ROUND = int(os.getenv("MAX_INDEXED_PER_ROUND", "0"))
    ...
    def _run_web_search_with_guard(q: str, preview_limit: int = 5, retries: int = 2) -> bool:
        nonlocal chunk_total, ingest_success, added_preview_urls, json_paths, new_docs_preview, queries
        def _is_bad_doc(d):
        ...
        for attempt in range(retries + 1):
            try:
                ...
                # 결과 JSON 이동: ./resources/<topic_slug>/
                try:
    ...
    def _fallback_auto_queries():
        ...
        extra = []
        outline_text_local = outline_text
        if outline_text_local:
            for line in outline_text_local.splitlines():
...
def research_synthesizer(state: State):
    ...
    if docs:
        ...
        for d in scored:
            ...
            src = meta.get("source") or meta.get("url") or "unknown"
            txt = _clean_snip(page_content or "", 420)
            key = (src, txt)
            if key in seen:
                continue
            seen.add(key)
...
"""
    ...
    if next_round < max_iter:
        if not has_pending(tasks, "research_planner", prefix="plan_"):
...
def chapter_writer(state: State):
    ...
    # 집필 대상 선택: 명시(write: ...) → 다음 미작성 제목 → 폴백
