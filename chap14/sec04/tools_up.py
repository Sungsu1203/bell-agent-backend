...
def is_written(title: str) -> bool:
    """해당 제목의 파일이 이미 있는지 확인"""
    return chapter_filepath(title).exists()

def next_unwritten_title(outline_text: str) -> str | None:
    """
    아직 작성되지 않은 목차 제목을 자동 선택
    우선순위: ## (2단계) 먼저 → 없으면 # (1단계)
    """
    headings = parse_outline_headings(outline_text)
    for level, title in headings:
        if level >= 2 and not is_written(title):
            return title
    for level, title in headings:
        if level == 1 and not is_written(title):
            return title
    return None
