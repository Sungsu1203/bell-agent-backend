...
def _search_google_cse(query: str, *, num: int = 10, timeout: int = 20) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # 👇 Google 전용 상한(환경변수로 조절 가능, 기본 8s)
...
def web_search(
    ...
    *,
    ...
    try:
        _backend_timeout = int(os.getenv("BACKEND_TIMEOUT_SEC", "15"))
    except Exception:
        _backend_timeout = 15
    if _backend_timeout < 5:
        _backend_timeout = 5
    # 일괄 안전 상한(응답성 향상): 8s
    if _backend_timeout > 8:
        logger.debug("[web_search] cap backend timeout: %ss → 8s", _backend_timeout)
        _backend_timeout = 8
