...
from content.api import find_section_path  # ← 후보 경로 탐색용 re-export 사용
...
def _load_section_body(root_dir: str, topic_slug: str, title: str) -> Optional[tuple[str, str, str]]:
    ...
    # ▼ 추가: 표준 패턴으로 못 찾으면 find_section_path()로 후보 슬러그 변형을 폭넓게 탐색
    try:
        p2 = find_section_path(title=title, topic_slug=topic_slug, mode=_as_doc_mode(None))
    except Exception as e:
        logger.debug("find_section_path failed (%s): %s", title, e)
        p2 = None
    if p2 and p2.exists():
        try:
            body2 = p2.read_text(encoding="utf-8")
            # used_dir는 통계 집계 상 'sections'로 취급(표준 저장 위치)
            return "sections", str(p2), body2
        except Exception as e:
            logger.debug("read_text failed for %s: %s", p2, e)
    return None
