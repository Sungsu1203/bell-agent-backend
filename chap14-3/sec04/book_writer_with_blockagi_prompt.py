...
from content_utils import (
    # 파일/경로
    read_outline, save_outline, save_md_draft,
    next_unwritten_title, parse_outline_headings,
    is_written, get_content_dir, path_for_title,
)

from rag_expression import (
    RE_WRITE_LINE,  # 유지: 일부 검색/백워드 호환
    extract_write_title,
    is_outline_creation,
    is_outline_display,
)
...
import os
import argparse
from utils.state_sanitize import coerce_int, sanitize_numeric_state

parser = argparse.ArgumentParser()
parser.add_argument("--iteration_count", type=str, default=os.getenv("ITERATION_COUNT", "2"))
...
initial_state: dict = {
    ...
    # 문자열/float/None 가능성을 모두 흡수
    "iteration_count": coerce_int(args.iteration_count, default=2),
    "research_round":  0,    # 시작은 0
    "new_url_count":   0,    # 필요 시 None 허용이면 coerce_int(None, 0, none_ok=True)
    "topic_slug":      os.getenv("TOPIC_SLUG") or "default",
    # ... 기타 키들
}

# 마지막으로 일괄 보정(중복 안전)
initial_state = sanitize_numeric_state(initial_state)
...
# ── 간단 상태 저장기(기존 utils.save_state 대체) ─────────────────────
...
def supervisor(state: State):
    ...
    if fqs:
        ...
        messages.append(AIMessage(f"[Supervisor fast-path] → web_search_agent (force_queries {len(fqs)}개 감지)"))
    ...
    if is_outline_creation(last_text):
        ...
        messages.append(AIMessage(f"[Supervisor fast-path] → content_strategist (target={fname})"))
        return {"messages": messages, "task_history": task_history}
    ...
    if target_from_line:
        ...
        if refs_empty or has_pending_rag:
            if not has_pending(task_history, "web_search_agent"):
                task_history.append(Task(agent="web_search_agent", done=False, description="rag_update:auto", done_at=""))
            messages.append(AIMessage("[Supervisor fast-path] 레퍼런스 비어있음/진행중 → RAG 먼저(web_search_agent)."))
            return {"messages": messages, "task_history": task_history}

        writer_agent = WRITER_AGENT
        task_history.append(Task(agent=writer_agent, done=False, description=f"write: {target_from_line}", done_at=""))
        messages.append(AIMessage(f"[Supervisor fast-path] → {writer_agent} (mode={DOC_MODE}, write: {target_from_line})"))
        return {"messages": messages, "task_history": task_history}
    ...
    if task.agent in ("chapter_writer", "section_writer"):
        ...
        if task.agent != expected:
            messages.append(AIMessage(f"[Supervisor reconcile] DOC_MODE={DOC_MODE} → writer agent forced to {expected} (from {task.agent})"))
...
def supervisor_router(state: State):
    ...
    if (state.get("agent_role") or "") == "research analyst" and (state.get("research_objectives") or []):
        # 초기 라운드를 research_planner로 시작
        state["task_history"] = [Task(agent="research_planner", done=False, description="plan_first", done_at="")]
...
def web_search_agent(state: State):
    ...
    if not has_pending(tasks, "vector_search_agent"):
        ...
        if queries: desc += f" queries={queries}"
        if json_paths: desc += f" json_paths={json_paths}"
...
def _score_doc(d: Document) -> float:
    ...
    score = 0.0
    if any(t in src for t in [".go.kr", ".gov", ".ac.kr", ".edu"]): score += 3
    if any(k in src for k in ["wsts", "imf", "kostat", "stat", "kotra"]): score += 2
    if any(k in src for k in ["kpmg","mckinsey","gartner","idc"]): score += 1.5
    y = re.search(r"(20\d{2})", src) or re.search(r"(20\d{2})", (getattr(d, "page_content","") or ""))
    if y:
        yr = int(y.group(1)); score += max(0, (2026 - yr) * 0.1)
    return score

from utils.state_sanitize import sanitize_numeric_state, as_int
...
from utils.state_sanitize import as_int, sanitize_numeric_state
def research_synthesizer(state: State):
    print("\n\n============ RESEARCH SYNTHESIZER ============")

    # --- (선택) 전역 상태 보정 유틸이 있으면 먼저 적용 ---
    try:
        from utils.state_sanitize import sanitize_numeric_state
        state = sanitize_numeric_state(dict(state))
    except Exception:
        pass  # 유틸이 없거나 에러여도 계속 진행

    # --- 안전한 값 읽기/캐스팅 ---
    rnd_raw = state.get("research_round", 0)
    try:
        rnd = int(rnd_raw)
    except Exception:
        rnd = 0
    ...
    iter_raw = state.get("iteration_count", 1)
    try:
    ...
    new_url_count_raw = state.get("new_url_count", None)
    try:
        new_url_count = int(new_url_count_raw) if new_url_count_raw is not None else None
    except Exception:
        new_url_count = None
    if isinstance(new_url_count, int) and new_url_count < 0:
        new_url_count = 0
...
"""
    ...
    # --- 라운드 증가 + 얼리 스톱 ---
    # 새 URL이 없거나 문서가 비어 있으면 추가 라운드 의미가 없으므로 종료로 보냄
    if (not docs) or (new_url_count is not None and new_url_count == 0):
        next_round = max_iter
    else:
...
def vector_search_agent(state: State):
    ...
    auto_title = next_unwritten_title(outline_text, mode=DOC_MODE, root_dir=current_path, topic_slug=state.get("topic_slug"))
    target_title = requested_title or auto_title or fallback_default
...
def chapter_writer(state: State):
    ...
    out_path = save_md_draft(
        target_title, gathered,
        mode="book", root_dir=current_path, topic_slug=state.get("topic_slug"),
    )
    ...
    if not has_pending(tasks, "communicator"):
        tasks.append(Task(agent="communicator", done=False, description=f"'{target_title}' 초안 완료 보고 및 다음 집필/수정 확인", done_at=""))
...
def section_writer(state: State):
    ...
    out_path = save_md_draft(
        target_title, gathered,
        mode="report", root_dir=current_path, topic_slug=state.get("topic_slug"),
    )
    ...
    try:
        ...
        if not any(x in base_text for x in ["chapters\\", "sections\\", "chapters/", "sections/"]):
            ...
            # 최근 메시지들에서 저장 경로 추적
                ...
                if m1:
                    last_save_path = (m1.group(1) or "").strip(); break
...
from utils.state_sanitize import as_int

rnd       = as_int(state, "research_round", 0)
max_iter  = as_int(state, "iteration_count", 1)
new_count = as_int(state, "new_url_count", 0, none_ok=True)

# 그래프 실행
state = graph.invoke(initial_state, config={"recursion_limit": 200})
# argparse에서 type=int로 받아도 되지만, 문자열 "3.0" 같은 케이스가 있으면 coerce_int가 더 안전
parser.add_argument("--iteration_count", type=str, default=os.getenv("ITERATION_COUNT", "2"))
...
def after_vector_router(state: State):
    ...
    has_objs = bool(state.get("research_objectives"))   # ✅ 연구 목표 존재 여부 체크

    # 연구 모드는 "연구 분석가" + "연구 목표가 있음" + "라운드 미완료"일 때만
    if role == "research analyst" and has_objs and rounds_done < max_iter:
        return "research_synthesizer"
    
    # 그 외엔 집필/커뮤니케이션 꼬리 라우팅
...
# graph_builder.add_edge("research_synthesizer", "research_planner")
def after_synthesizer_router(state: State):
    rounds_done = int(state.get("research_round") or 0)
    max_iter = int(state.get("iteration_count") or 0)
    print(f"[ROUTER] after_synthesizer: rounds_done={rounds_done}, max_iter={max_iter}")
    # 연구 라운드가 남아있을 때만 다음 라운드 계획 수립으로,
    # 아니면 바로 집필/커뮤니케이션으로 탈출
    if rounds_done < max_iter:
...
def _initial_state() -> State:
    return State(
        ...
        iteration_count=int(os.getenv("BLOCKAGI_ITERATION_COUNT", "3") or "3"),
