...
def _save_results(items, out_dir: Optional[Path | str] = None, *, query: Optional[str] = None) -> str:
    ...
    out_dir 기본값:
      - ENV WEB_RAG_DATA_DIR
      - 없으면 DATA_DIR
    파일명: resources_{timestamp}_{hash(query, 4bytes)}.json
    """
    base_dir = Path(os.getenv("WEB_RAG_DATA_DIR", "")) if out_dir is None else Path(out_dir)
    if not str(base_dir):
        base_dir = DATA_DIR
...
def web_results_to_documents(resources: List[Dict[str, Any]]) -> List[Document]:
    ...
    for r in (resources or []):
        ...
        if not pc or _is_block_page(pc) or _looks_like_pdf_bytes(pc):
            continue
        docs.append(Document(page_content=_clean_text(pc), metadata={"title": r.get("title", ""), "source": r.get("url", "")}))
    return docs

def web_page_json_to_documents(json_file: str) -> List[Document]:
    """
    저장된 JSON 결과(resources_*.json) 파일을 LangChain Document로 변환.
    - 차단/봇 페이지/바이너리 감지 시 제외
    """
...
def documents_to_chroma(
    ...
    *,
    ...
    for d in (documents or []):
        ...
        if not txt or _is_block_page(txt) or _looks_like_pdf_bytes(txt):
            continue
    ...
    if all_urls:
        try:
            res = vs._collection.get(where={"source": {"$in": list(all_urls)}}, include=["metadatas"])
