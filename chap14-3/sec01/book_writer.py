...
def supervisor(state: State):
    ...
    messages = state.get("messages",[])        # 상태에서 메시지를 가져옴
...
# Edges
graph_builder.add_edge(START, "supervisor") # edge 변경 및 추가
...
# graph.get_graph().draw_mermaid_png(output_file_path=absolute_path.replace('.py', '.png'),max_retries=5, retry_delay=2.0,draw_method=MermaidDrawMethod.PYPPETEER)
