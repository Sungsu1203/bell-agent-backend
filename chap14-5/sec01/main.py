...
def _state_as_mapping(s: Any) -> dict[str, Any]:
    if s is None:
        return {}
    if isinstance(s, dict):
        return s
    # Pydantic v2
    if hasattr(s, "model_dump") and callable(s.model_dump):
        return s.model_dump()  # type: ignore[no-any-return]
    # Pydantic v1
    if hasattr(s, "dict") and callable(s.dict):
        return s.dict()        # type: ignore[no-any-return]
    # dataclass
    if is_dataclass(s):
        return asdict(s)
    # 일반 객체
    return vars(s) if hasattr(s, "__dict__") else {}
...
def _score_doc(d: Document, year_now: int | None = None) -> float:
    if year_now is None:
    ...
    def _norm_domain(url: str) -> str:
        ...
        if not netloc:  # 스킴이 없을 때 보정
            netloc = urlparse("http://" + url).netloc
        netloc = netloc.split("@")[-1]  # userinfo 제거
        netloc = netloc.split(":")[0]   # 포트 제거
        # www., m. 같은 흔한 프리픽스 제거
...
def _normalize_outline_headings(s: str) -> str:
    """
    1) '### 세부 목차 제안' 제거
    2) '1. 제목' 형식의 번호 리스트를 '## 1. 제목' H2 헤딩으로 승격
    3) (옵션) 불릿('- 제목')도 H2로 승격하며 번호를 1부터 부여
    """
    ...
    def _run_web_search_with_guard(q: str, preview_limit: int = 5, retries: int = 2) -> bool:
        ...
        for attempt in range(retries + 1):
            try:
                ...
                # [ANCHOR: ROUND_URL_TALLY_CALL]
                # 필터링된 결과 기준으로 '신규 URL' 집계
...
# 필요한 최소 import (파일 상단이나 본 함수 위에 배치)
from typing import Mapping, Any
from urllib.parse import urlparse, urlunparse

# ── 폴백 헬퍼 (외부에 이미 정의돼 있으면 이 부분은 생략 가능) ──────────────
def _canonicalize_src_for_dedup(src: str | None) -> str:
    ...
    try:
        ...
        if pu.scheme and pu.netloc:
            # query/fragment 제거, path는 끝 슬래시 정리
            path = pu.path or ""
...
def _auto_footnote_label(meta: dict, url: str) -> str:
    ...
    try:
        ...
        if pu.scheme and pu.netloc:
            # 파일명 우선, 없으면 도메인
            name = Path(pu.path or "").name
            return name or pu.netloc
    except Exception:
        pass
    return Path(url).name or "reference"
# ─────────────────────────────────────────────────────────────────────────────

# ── NEW: 다양한 형태의 refs 엔트리에서 메타데이터 추출 ─────────────
def _extract_meta(doc) -> dict:
    """
    refs 항목이 LangChain Document, dict, 혹은 중첩된 {"metadata": {...}} 등
    다양한 스키마로 들어와도 {url|source, title...}를 최대한 꺼내준다.
    """
    m = None
    # 1) dict 계열
    if isinstance(doc, dict):
        # 최상위에 url/source 있으면 그대로 사용
        if ("url" in doc) or ("source" in doc):
            ...
            if isinstance(md, dict):
                ...
                m = doc  # 혹시 모를 변종 대비 (마지막 안전망)
    else:
        # 2) 객체 계열 (예: LangChain Document, Dummy 객체)
        m = getattr(doc, "metadata", {}) or {}
        # 흔한 중첩: {"metadata": {"url":..., "source":...}}
        if isinstance(m, dict) and ("url" not in m and "source" not in m):
...
def attach_auto_citations(gathered: str, state: Mapping[str, Any] | None = None) -> str:
    ...
    if os.getenv("AUTO_FOOTNOTE_INLINE", "0") == "1":
        try:
            ...
            domain_map = {k: _netloc(u) for k, u in url_by_key.items()}
...
def chapter_writer(state: State):
    ...
    if os.getenv("AUTO_FOOTNOTE", "1") == "1":
        try:
            ...
            state_map = _state_as_mapping(state)
            gathered = attach_auto_citations(gathered, state_map)
        except Exception as e:
...
def after_synthesizer_router(state: State):
    ...
    print(f"[ROUTER] after_synthesizer: rounds_done={rounds_done}, max_iter={max_iter}, "
          f"new_url_count={url_new_actual}, halt_threshold={halt_threshold}, "
    ...
    # 2) “신규 없음” 조기 종료 → writer
    if should_halt:
        print(f"[ROUTER] halt: new_url_count<=threshold for {streak} round(s) → writer")
        return WRITER_AGENT
    
    # 3) 라운드 소진(=연구 종료) → writer (communicator 우선 아님)
    if rounds_done >= max_iter:
        print("[ROUTER] research finished → writer")
        return WRITER_AGENT
